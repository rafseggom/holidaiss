...
//		String query = request.getParameter("searchQuery");
		...
		log.log(Level.FINE, "Searching in Amadeus and Mapbox: " + destino);

		// Search for place in TomTom
		TomTomResource Tomtom = new TomTomResource();
		FuzzySearch TomTomResults = Tomtom.getFuzzySearch(destino);
		...
		log.log(Level.FINE, "Searching for airports or cities that contain " + destino);
...
//		HotelOffer[] AmadeusHotel = null;
		try {
			...
			AmadeusPlace = Amadeus.getAirportCity("MAD");
			AmadeusResults = Amadeus.getVuelos("MAD", precio, date);
			AmadeusCheap = Amadeus.getCheapestFlight("MAD", destino, precio, date);
...
//			request.setAttribute("hotels", AmadeusHotel);
		...
		request.getSession().setAttribute("results", destino);
